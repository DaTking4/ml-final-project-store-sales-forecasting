...
|---|---:|---:|---:|---|
| **baseline** — `changepoint=0.05, seasonality=10.0, holidays=10.0, additive, yearly=10` | `1,216.90` | **`1,855.10`** | `+52.4%` | overfit |
| `underfit_1` — `changepoint=0.001, seasonality=0.1, holidays=1.0, additive, yearly=5` | `1,867` | `2,792` | `+49.6%` | overfit |
...
| Validation WMAE | `1,855.10` |

საინტერესოა, რომ "underfit"-ად დაგეგმილმა კონფიგურაციამაც `fit_status = overfit` მიიღო — Prophet-ის gap-ზე დაფუძნებული classifier-ისთვის `train_wmae` (ერთი in-sample გავლის შედეგი) სისტემატურად დაბალია `val_wmae`-ზე, ამიტომ თითქმის ყველა run "overfit"-ად აღირიცხება — ეს classify_fit_status-ის ზოგადი შეზღუდვაა და არა Prophet-სპეციფიკური პრობლემა.

### დასკვნა

Prophet-ის სვიფი ბოლომდე ვერ დასრულდა, ამიტომ ქვემოთ მოცემული შედარება baseline-ს ეყრდნობა და არა hyperparameter-ით ოპტიმიზირებულ საბოლოო მოდელს — შედეგი ამიტომ ოპტიმისტურადაც შეიძლება ჩაითვალოს დანარჩენ, სრულად swept მოდელებთან შედარებით:

| მოდელი | Val WMAE |
|---|---:|
| XGBoost | `1,709.59` |
| TimesFM (zero-shot) | `1,815.32` |
| **Prophet (baseline, incomplete sweep)** | `1,855.10` |
| N-BEATS | `1,858.23` |
| LightGBM | `1,864.71` |
| TFT | `2,216.19` |
| PatchTST | `2,331.97` |
...
მიუხედავად იმისა, რომ hyperparameter sweep არ დასრულდა, Prophet-ის untuned baseline-იც კი კონკურენტუნარიანია — ეს მიანიშნებს, რომ ოთხი Walmart holiday-ს ცალკე `holidays` კომპონენტად გადაცემა (Super Bowl, Labor Day, Thanksgiving, Christmas) ძლიერ სიგნალს იძლევა ამ სეზონურ retail dataset-ზე, თუნდაც სრული hyperparameter tuning-ის გარეშე.
...
TimesFM აჯობებს DLinear-ს, TFT-ს, PatchTST-ს და ARIMA-ს, და Prophet-ს, N-BEATS-სა და LightGBM-ს ძალიან ახლოს უდგება — ეს შთამბეჭდავია იმის გათვალისწინებით, რომ მოდელი Walmart-ის data-ზე სწავლას საერთოდ არ ახდენდა. ეს ადასტურებს foundation model-ების პოტენციალს domain-specific time-series ამოცანებში.
